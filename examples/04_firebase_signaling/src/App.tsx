...
const DEFAULT_CHANNEL = "signaling/game-room";
...
export default function App() {
  ...
  const [channelPath, setChannelPath] = useState(DEFAULT_CHANNEL);
  ...
  const handleStart = async () => {
    if (!selfId.trim() || !apiKey.trim() || !databaseUrl.trim()) return;
    ...
    try {
      ...
      await t.start();
      setIsStarted(true);
      log("info", `Started as ${selfId}, Firebase channel: ${channelPath}`);
    } catch (err) {
        ...
        <div className="w-full max-w-sm space-y-6 bg-card p-6 rounded-lg border border-border shadow-sm">
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Firebase Signaling + P2P Transport
            </p>
            <p className="text-[10px] text-muted-foreground">
              Completely serverless — no backend required
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="signaling/game-room"
                className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring font-mono"
              />
            </div>
            <div className="space-y-2">
            ...
            >
              {isConnecting ? "Connecting..." : "Join via Firebase"}
            </button>
            {connectStatus && (
          ...
          <div className="bg-card rounded-lg border shadow-sm p-6 space-y-4">
            ...
            <div className="flex items-center gap-2 p-3 rounded-md bg-muted/50 border">
              ...
              <p className="text-xs text-muted-foreground">
                Peers are discovered automatically via Firebase Realtime
                Database. Completely serverless.
              </p>
            </div>
            {connectStatus && (
